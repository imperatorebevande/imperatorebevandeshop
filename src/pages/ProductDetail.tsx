...
const ProductDetail = () => {
  ...
  const product = {
    ...
    originalPrice: wooProduct.regular_price && wooProduct.sale_price && parseFloat(wooProduct.regular_price) > parseFloat(wooProduct.sale_price) ? parseFloat(wooProduct.regular_price) : undefined,
    ...
    features: ['Prodotto di qualità', 'Consegna rapida a Bari', 'Garanzia di freschezza']
  };
  ...
  const discountPercentage = product.originalPrice ? Math.round((product.originalPrice - product.price) / product.originalPrice * 100) : null;
  return <div className="min-h-screen bg-gray-50">
      <Header />
          ...
          <CardContent className="p-6">
            <div className="flex gap-6">
              ...
              <div className="flex-shrink-0 relative">
                <img src={product.image} alt={product.name} className={`w-[150px] h-[150px] object-cover rounded-lg bg-white p-2 ${!product.inStock ? 'grayscale opacity-75' : ''}`} onError={e => {
                e.currentTarget.src = '/placeholder.svg';
              }} />
                {!product.inStock && <Badge className="absolute top-2 left-2 bg-red-500 text-white">
                    Non Disponibile
                  </Badge>}
                {product.inStock && discountPercentage && <Badge className="absolute top-2 left-2 bg-red-500 text-white">
                    -{discountPercentage}%
                  </Badge>}
              </div>
              ...
              <div className="flex-1">
                ...
                {/* Rating */}
                {product.rating > 0 && <div className="flex items-center mb-3">
                ...
                <div className="mb-4">
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl font-bold text-sky-600 text-center">
                      €{product.price.toFixed(2)}
                    </span>
                    {product.originalPrice && <span className="text-lg text-gray-500 line-through">
                        €{product.originalPrice.toFixed(2)}
                      </span>}
                  </div>
                  {discountPercentage && product.inStock && <span className="text-green-600 font-medium text-sm">
                ...
                {/* Stock Status */}
                <div className="mb-4">
                  {product.inStock ? <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                      ✓ Disponibile
                  ...
                  {product.inStock ? <>
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex items-center border rounded-md">
                          <Button variant="ghost" size="sm" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 h-8">
                            -
                          </Button>
                          <span className="px-3 py-1 border-x text-sm">{quantity}</span>
                          <Button variant="ghost" size="sm" onClick={() => setQuantity(quantity + 1)} className="px-3 h-8">
                            +
                          </Button>
                        </div>
                      </div>

                      <Button className="gradient-primary hover:opacity-90" onClick={handleAddToCart}>
