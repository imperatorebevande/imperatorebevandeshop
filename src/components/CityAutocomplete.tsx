...
interface CityAutocompleteProps {
  ...
  required?: boolean;
  error?: boolean;
  province?: string; // Per filtrare le città per provincia
}
...
const CityAutocomplete: React.FC<CityAutocompleteProps> = ({
  ...
  const searchCities = async (query: string) => {
    ...
    try {
      const request = {
        ...
        ...(province && { bounds: undefined }) // Potremmo aggiungere bounds per provincia
      };
      ...
      autocompleteService.current.getPlacePredictions(request, (predictions: any, status: any) => {
        ...
        if (status === (window as any).google.maps.places.PlacesServiceStatus.OK && predictions) {
          ...
          const mappedSuggestions = filteredPredictions.map(prediction => ({
  ...
  const selectSuggestion = async (suggestion: CitySuggestion) => {
    ...
    try {
      ...
      console.error('Errore nel recupero dei dettagli della città:', error);
      setTimeout(() => {
        setIsSelecting(false);
      }, 300);
    }
